jQuery(function($) {
	...
	/*setInterval(function(){img_03_left();}, 900);
	setInterval(function(){img_03_right();}, 900);
	setInterval(function(){img_slick_left();}, 900);
	setInterval(function(){img_slick_right();}, 900);*/
...
function img_slick_left(){
	setTimeout(function() {
		$('.img_slick_left').css('right',50);
		$('.img_slick_left').stop().animate({left:10}, 1000, 'easeOutElastic');
	}, 900);
}
function img_slick_right(){
	setTimeout(function() {
		$('.img_slick_right').css('right',10);
		$('.img_slick_right').stop().animate({right:10}, 1000, 'easeOutElastic');
	}, 1400);
}
function char_teacher(){
	setTimeout(function() {
		$('.char_teacher').css('bottom',10);
		$('.char_teacher').stop().animate({right:10}, 1000, 'easeOutElastic');
	}, 2000);
}
function img_03_left(){
	setTimeout(function() {
		$('.img_03_left').css('right',50);
		$('.img_03_left').stop().animate({right:10}, 1000, 'easeOutElastic');
	}, 900);
}
function img_03_right(){
	setTimeout(function() {
		$('.img_03_right').css('right',10);
		$('.img_03_right').stop().animate({right:10}, 1000, 'easeOutElastic');
	}, 1400);
}
...
$(window).scroll(function(){
	...
	if(scr <= 200){
		$('#wrap .join_event h3 > img.tit').stop().animate({left:-150,opacity:0}, 0);
		$('#wrap .join_event h3 > img.txt').stop().animate({right:-150,opacity:0}, 0);
		$('#wrap .join_event .animated').addClass('fadeInDown');
		$('#wrap .join_event .animated').removeClass('fadeInUp');
		$('#wrap .app_sns .bubble').addClass('fadeInDown');
		$('#wrap .app_sns .bubble').removeClass('fadeInUp');

		$('#wrap h2.tit_email').addClass("motion");
		$('#wrap div.event_con h2').addClass("motion");
		$('#wrap .img_screenshot .txt_sshot').addClass("motion");
	}

	if(scr > 1000 && scr < 3500) {
		$('#wrap .join_event h3 > img.tit').stop().animate({left:120,opacity:1}, 600);
		$('#wrap .join_event h3 > img.txt').stop().animate({right:68,opacity:1}, 800);

		$('#wrap .join_event .animated').removeClass('fadeInDown');
		$('#wrap .join_event .animated').addClass('fadeInUp');
	}else if(scr > 3700){
		$('#wrap .app_sns .bubble').removeClass('fadeInDown');
		$('#wrap .app_sns .bubble').addClass('fadeInUp');
	}
	if(scr > 400 && scr < 899 ) {
		if($('#wrap h2.tit_email').hasClass("motion")){
			$('#wrap h2.tit_email').stop().animate({top:20}, 200, function() {
				$('#wrap h2.tit_email').stop().animate({top:0}, 200);
			});
			$('#wrap h2.tit_email').removeClass("motion");
		}
	}else if(scr > 900  && scr < 2599) {
		if($('#wrap div.event_con h2').hasClass("motion")){
			$('#wrap div.event_con h2').stop().animate({top:20}, 200, function() {
				$('#wrap div.event_con h2').stop().animate({top:0}, 200);
			});
			$('#wrap div.event_con h2').removeClass("motion");
		}
	}else if(scr > 2600) {
	...
	if(scr > 600) {
		setTimeout(function() {
			$('.img_03_left').stop().animate({left:10}, 1000, 'easeOutElastic');
		}, 200);
		setTimeout(function() {
			$('.img_03_right').stop().animate({right:10}, 1000, 'easeOutElastic');
		}, 400);
		setTimeout(function() {
			$('.char_teacher').stop().animate({bottom:'9%'}, 1000, 'easeOutElastic');
		}, 700);
	}
	if(scr > 1200) {
		setTimeout(function() {
